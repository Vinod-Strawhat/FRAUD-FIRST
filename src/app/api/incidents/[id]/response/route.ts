...
export async function GET(
  _request: Request,
  ctx: RouteContext<"/api/incidents/[id]/response">
): Promise<Response> {
  ...
  if (!isValidIncidentId(id)) {
    return Response.json(
      ...
      } satisfies GetResponseResult,
      { status: 400 }
    );
  }

  try {
    const response = await getIncidentResponse(id);
    return Response.json({ ok: true, response } satisfies GetResponseResult, {
      status: 200,
    });
  } catch (error) {
